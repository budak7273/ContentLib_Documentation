...
* `Wrong Naming Convention ! "Recipe" expected followed by a Name`
  ** Your recipe file should be named like this: `Recipe_YourNameHere.json`
* Recipe not showing up in-game
  ** Did you give your Item or Recipe a Category? If not, it will only show up when searched for.
  ** Have you unlocked the schematic (or other source) responsible for granting access to the recipe?
...
* Selecting the recipe in a machine immediately clears the machine's selected recipe or shows UNDEF item names
  ** Make the sure the machine has the enough input/output ports for the Form of ingredients and products you defined. Machines without fluid ports can't use fluids, for example.
