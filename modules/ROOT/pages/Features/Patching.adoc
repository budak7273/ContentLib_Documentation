...
Here's an example schematic patch that unlocks the "Uranium" schematic when the "Biocoal" schematic is unlock:
...
To unlock geysers, use `Geyser` as the node type and `Desc_Geyser_C` as the resource.
