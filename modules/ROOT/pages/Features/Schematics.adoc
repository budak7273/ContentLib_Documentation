...
- A base-game bug causes HUB Milestones without icons to display improperly,
  so if you're creating a new HUB milestone, you should always give it an icon by specifying a xref:Features/VisualKits.adoc[VisualKit].
...
Notice that the schematics' file names are numbered to ensure the hidden one is processed before the visible one to avoid link:https://github.com/Nogg-aholic/ContentLib/issues/8[this bug].
